    ...
    <div className="bg-card border border-border rounded-xl p-4 sm:p-6 min-w-0">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right min-w-0">
          ...
          <div className="text-lg sm:text-xl font-mono font-semibold mt-0.5 break-all">
...
function PositionRowView({ row, color }: { row: PositionRow; color: string }) {
  ...
  if (isMobile) {
    return (
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full text-left rounded-md px-2 -mx-2 py-2 min-h-11 transition-colors hover:bg-muted/40 cursor-pointer"
      >
        {body}
      </button>
    );
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="top" align="end" sideOffset={12} collisionPadding={8} className="text-xs">
